...
"""
CNN on mnist data using fluid api of paddlepaddle use GaussianAugmentationDefence
"""
...
from adversarialbox.defences.gaussian_augmentation import GaussianAugmentationDefence
...
def main(use_cuda):
    ...
    for pass_id in range(PASS_NUM):
        pass_acc.reset()
        for datas in train_reader():
            #使用GaussianAugmentationDefence加强训练
            x=[]
            y=[]
            ...
            x_gad,y_gad=GaussianAugmentationDefence(np.array(x),np.array(y),std=0.3,r=1.)
            ...
            for i, a in enumerate(x_gad):
                new_datas.append([a,y_gad[i]])

            loss, acc, b_size = exe.run(
                fluid.default_main_program(),
                feed=feeder.feed(new_datas),
    ...
    fluid.io.save_params(
        exe, dirname='./mnist-gad', main_program=fluid.default_main_program())
